...
namespace UER{
	...
	bool CWaveFile::Open(const wchar_t* fileName)
	{
		...
		if (m_hmmio == NULL) {
			UER_WARNING_MB("waveファイルのオープンに失敗しました。filePath : %s\n"
								   "原因として下記の２点が考えられます。\n"
								   "①　ファイルパスが間違っている。\n"
								   "②　ファイルがAssetsフォルダの中にない。\n"
								   "上記２点を確認して、問題がない場合は一度VisualStudioのビルド/リビルドを行ってみてください。\n", fileName);
			UER_WARNING_MB("Failed mmioOpen");
			return false;
		}
		...
		if ((0 != mmioDescend(m_hmmio, &m_ckRiff, NULL, 0))) {
			UER_WARNING_MB("Failed mmioDescend");
			Release();
			return false;
		}
		...
		if (mmioRead(m_hmmio, (HPSTR)&pcmWaveFormat,
			...
			UER_WARNING_MB("Failed mmioRead");
			Release();
			return false;
		}
		...
		{
			UER_WARNING_MB("Failed mmioAscend");
			Release();			
			return false;
		}
		ResetFile();
	...
	void CWaveFile::ResetFile()
	{
		...
		if (-1 == mmioSeek(m_hmmio, m_ckRiff.dwDataOffset + sizeof(FOURCC),
			SEEK_SET)) {
			UER_WARNING_MB("Failed mmioSeek");
			return ;
		}
		...
		for (DWORD cT = 0; cT < cbDataIn; cT++)
		{
			...
			if (mmioinfoIn.pchNext == mmioinfoIn.pchEndRead)
			{
				if (0 != mmioAdvance(m_hmmio, &mmioinfoIn, MMIO_READ)) {
					UER_WARNING_MB("mmioAdvance");
					return ;
				}

				if (mmioinfoIn.pchNext == mmioinfoIn.pchEndRead) {
					UER_WARNING_MB("mmioinfoIn.pchNext");
					return;
				}
			}
