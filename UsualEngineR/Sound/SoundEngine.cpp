...
namespace UER{
	...
	void CSoundEngine::Init()
	{
		...
		if (FAILED(hr = XAudio2Create(&m_xAudio2, flags)))
		{
			...
			UER_WARNING_MB( "Faild XAudio2Create");
		...
		{
			Release();
			UER_WARNING_MB( "Faild CreateMasteringVoice");
			MessageBox(nullptr, L"オーディオデバイスの初期化に失敗しました", L"エラー", MB_OK);
			return;
		}
		...
		if (FAILED(hr = XAudio2CreateReverb(&m_reverbEffect, flags))) {
			Release();
			UER_WARNING_MB( "Faild XAudio2CreateReverb");
			MessageBox(nullptr, L"オーディオデバイスの初期化に失敗しました", L"エラー", MB_OK);
			return;
		}
		//サブミックスボイスを作成。
		...
		{
			Release();
			UER_WARNING_MB( "Faild CreateSubmixVoice");
			MessageBox(nullptr, L"オーディオデバイスの初期化に失敗しました", L"エラー", MB_OK);
			return ;
		}
		//デフォルトのFXパラメータを設定。
	...
	*/
	...
	{
		UER_ASSERT(waveFile->GetFormat()->nChannels <= INPUTCHANNELS, "Channel over");
		IXAudio2SourceVoice* pSourceVoice;
		if (is3DSound == false) {
			//2Dサウンド。
			if (FAILED(m_xAudio2->CreateSourceVoice(&pSourceVoice, waveFile->GetFormat())))
			{
				UER_WARNING_MB("Failed CreateSourceVoice");
				return nullptr;
			}
		}
		else {
